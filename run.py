...
def inference(model, inputs, batch_size, return_logits=False, bad=False):
    ...
    for batch in tqdm(dataloader):
        ...
        with torch.no_grad():
            loss = run_model(model, input_ids, attention_mask, token_type_ids, labels=labels,
                             return_logits=return_logits, bad=bad)
...
def run_model(model, input_ids, attention_mask, token_type_ids,
              aux_weight=0.0, target_indices=None, labels=None, return_logits=False, 
              bad=False):
    ...
    if target_indices != None:
        layer = model.transformer.wte if torch.cuda.device_count()==1 else model.module.transformer.wte
    ...
    return torch.sum(losses, axis=1) / torch.sum(label_mask, axis=1) * (-1 if bad else 1) + aux_loss
