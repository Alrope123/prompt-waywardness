...
def main():
    ...
    parser.add_argument("--task", type=str, nargs="+",
        default=['SST-2', 'sst-5', 'subj', 'trec', 'agnews'],
        help="Task names")
    ...
    main_for_gao(args, [task for task in args.task
                        if task in ["SST-2", "sst-5", "trec", "trec-5", "trec-4", "trec-3", "subj"]])
...
def prepro_for_zhang(dataname, split, seed, args):
    ...
    label_set = set()
    with open(os.path.join(args.data_dir, "agnews", "{}.csv".format(split)), "r") as f:
